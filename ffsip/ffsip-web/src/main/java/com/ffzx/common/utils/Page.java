...
public class Page implements Serializable {
     ...
     * 查询水位
     **/
    private int waterLine = DEFAULT_WATER_LINE;//add by hyl  app做分页查询时候需要此参数
    /**
     * 总记录数
     **/
    ...
    //该方法用于分页记录补位用途使用,目前app分页需用到此功能
    public Page(int pageIndex, int pageSize, int waterLine) {
        // check:
        if (pageIndex < 1) {
            pageIndex = 1;
        }
    ...
    public void setTotalCount(long mytotalCount) {
        ...
        if (totalCount == 0) {
            if (pageIndex != 1) {
                //为了配合APP端的分页，不需要这个判断(cmm 2016-06-25)
    ...
    public int getFirstResult() {
        int offset = ((pageIndex - 1) * pageSize) + waterLine;
        if (offset < 0) {
            offset = 0;
        }
        return offset;
